# Shell Navigation

**Area:** Shell - Icon Rail + Expansion Panel + Topbar  
**Trigger:** User is authenticated and the dashboard layout is mounted  
**Required Permission:** Any authenticated user (individual pillars gated per permission - see [[frontend/design-system/components/nav-rail|Nav Rail]])

---

## Preconditions

- User is authenticated (valid JWT)
- Dashboard layout is rendered: topbar, rail, optional panel, content area
- Active section is set from URL or defaults to `home` on first load

---

## Flow 1 - Clicking a Section WITH a Panel

**Example:** User clicks "Work" on the rail

### Step 1: Rail Item Becomes Active

- **UI:** Clicked pillar item background -> `rgba(255,255,255,0.10)`, text color -> `rgba(255,255,255,0.95)`. Previously active item returns to default state.
- **State change:** `activeSection = 'work'`, `isPanelOpen = true`

### Step 2: Expansion Panel Opens

- **UI:** Panel shell animates from `width: 0, opacity: 0` -> `width: 210px, opacity: 1` over `220ms cubic-bezier(0.16,1,0.3,1)`. Panel title updates to "Work". Panel body renders the pillar's sub-navigation items.
- **Default active item:** First item in the list is pre-highlighted in accent color (`#ECEAFD` bg, `#5B4FE8` text). Navigation routes to that item's route.
- **API:** None - panel content is static config, not fetched.

### Step 3: Breadcrumb Updates

- **UI:** Topbar breadcrumb updates to: `[Company Name] / Work / Projects` (pillar + first active panel item)

### Step 4: Content Area Navigates

- **UI:** Content area renders the default route for the section (e.g., `/work` -> Projects page)

---

## Flow 2 - Clicking a Panel Item

**Example:** User clicks "Projects" under Work

### Step 1: Panel Item Becomes Active

- **UI:** Clicked item -> `bg-[#ECEAFD] text-[#5B4FE8] font-medium`. Previous active item returns to default.

### Step 2: Breadcrumb and Content Update

- **UI:** Breadcrumb -> `[Company Name] / Work / Projects`. Content area navigates to `/work/projects`.
- **Rail:** Work item remains active (clicking a panel item does not change the active pillar).

---

## Flow 3 - Clicking a Section WITHOUT a Panel

**Sections with no panel:** Home (`/`), Inbox (`/inbox`)

### Step 1: Rail Item Becomes Active

- **UI:** Clicked item highlighted. Previous item returns to default.
- **State change:** `activeSection = 'home'`, `isPanelOpen = false`

### Step 2: Panel Closes

- **UI:** If a panel was open: animates to `width: 0, opacity: 0` over `220ms`. No panel renders for no-panel pillars.
- **Breadcrumb:** Updates to `[Company Name] / Home` - only two levels (no third level without a panel).

### Step 3: Content Navigates Directly

- **UI:** Content area navigates to the section's direct route.

---

## Flow 4 - + Create Dropdown

**Trigger:** User clicks the `+` button in the panel head

### Step 1: Dropdown Opens

- **UI:** Dropdown appears absolutely positioned below the `+` button (`top: calc(100% + 4px), right: 0`). Entry animation: fade + `translateY(-6px) scale(0.97)` -> normal over `140ms`. Lists contextual create actions for the current section.

### Step 2: User Selects an Action or Dismisses

- **Select action:** Dropdown closes, action is triggered (open create modal / navigate to create page).
- **Click outside:** `document` click listener closes the dropdown.

---

## Flow 5 - Tenant / Legal Entity Context in Topbar

**Trigger:** User clicks the company name in the topbar

### Step 1: Dropdown Opens

- **UI:** Dropdown shows current tenant and legal entity context, Legal Entities, and Connected Companies when permitted. It must not present connected companies as legal-entity/session switching.

### Step 2: User Opens Legal Entities Or Connected Companies

- **Legal Entities:** Navigates to legal entity setup for single-company or multi-company structure.
- **Connected Companies:** Opens permission-scoped connected-company views or management. Auth session remains anchored to the current company tenant.
- **Audit:** Cross-company views and actions are audit logged with requester tenant and source tenant.

### Step 3: Static Label For Basic Users

- **UI:** Company name renders as a static label when the user has no legal-entity or connected-company permissions.

---

## Active State Rules

| Element | Active condition |
|:--------|:----------------|
| Rail item | Current URL is within that section's route segment |
| Panel item | Current URL exactly matches that item's route |
| Both rail + panel | When a sub-page is active, both the section AND the specific panel item are highlighted |

---

## Error Scenarios

| Scenario | Behaviour |
|:---------|:----------|
| User navigates directly to a URL (e.g., `/work/projects`) | Rail highlights Work, panel opens with Projects active - state is derived from URL |
| Permission revoked mid-session | On next navigation, pillar is hidden and user is redirected to first accessible route |
| Section has no items the user can see (all permission-gated) | Panel still opens but body is empty - no items render |

---

## Related

- [[frontend/design-system/components/nav-rail|Nav Rail]] - icon rail spec (dimensions, icons, states)
- [[frontend/design-system/components/expansion-panel|Expansion Panel]] - panel spec (dimensions, animation, items)
- [[frontend/architecture/topbar|Topbar]] - topbar spec including breadcrumb and company context
- [[frontend/architecture/sidebar-nav|Sidebar Nav Map]] - all pillar routes and permission keys
- [[Userflow/Dashboard/dashboard-overview|Dashboard Overview]] - what renders in the content area on load

