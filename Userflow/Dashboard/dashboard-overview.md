...
- **API:** Three parallel calls on mount:
  1. `GET /api/v1/dashboard` - returns `{ enabledZones: string[] }` (server-filtered by backend session permissions)
...
- **UI:** No visible change - computation is client-side. Zones are merged: server's `enabledZones` list (what exists) x user's saved pref order (how they've arranged it). Result: ordered list of zones to render. Zone 1 (Exception Alert Strip) always pinned first if present - cannot be reordered
