...
**Wizard route:** `/tenants/new` with sub-routes `/tenants/new/step-1`, `/tenants/new/step-2`, `/tenants/new/step-3`, `/tenants/new/step-4`, `/tenants/new/review`.
...
| Internal values | `1-10`, `11-50`, `51-200`, `201-500`, `501-1000`, `1001-5000`, `5000+` |
| Note | This range is stored and drives subscription plan price bracket selection in Step 3. |
| Error — empty | "Company size is required." |
...
|---|---|---|
| Cancel | Bottom-left | Opens confirmation dialog: "Cancel tenant creation? Any unsaved data will be lost." → OK navigates to `/tenants`. |
...
**State written:**
- New row in `tenants`: `id`, `tenant_code`, `company_name`, `legal_company_name`, `domain`, `industry`, `company_size_range`, `description`, `phone_number`, `website`, `status = 'provisioning'`, `created_at`, `created_by` (platform account ID)
...
| Label | "Billing Cycle" |
| Type | Radio button group |
| Required | Yes — only when Commercial Model = Subscription |
| Visible | Only when Commercial Model = Subscription |
| Options | Monthly, Annual |
| Note | Annual billing shows a calculated total. Price is the sum of selected module bracket prices × 12 before any override. |
...
| Source | Determined by selected plan's `included_module_keys` array — pre-checked modules from plan cannot be unchecked |
| Additional modules | Operator can add purchasable add-on modules on top of plan-included modules |
| Price effect | Selecting/deselecting add-on modules recalculates the price shown below. Price is calculated live from `module_catalog.price_brackets` for the selected company size band |
...
|:---|:---|:---|
| **HR Management (Package 1)** | Core HR, Leave Management, Calendar | Performance, Payroll, Skills, Learning, Recruitment, Grievance, Expense are Phase 2 — not shown |
| **Workforce Intelligence (Package 1)** | Activity Monitoring, Workforce Presence, Identity Verification, Exception Engine, Productivity Analytics | App Usage Tracking is part of Activity Monitoring. Agent Gateway is infrastructure — not a sellable module. |
...
**Workflow / Automation Engine:** Foundation module — always included, not shown as a selectable item. Automatically provisioned on activation. Powers leave approval, expense approval, attendance correction, exception escalation, and performance review routing. Tenant admins configure workflow definitions post-activation.
...
| Value | Sum of all selected module bracket prices for selected company size range |
| Updates | Live recalculation on module selection change or company size change |
| Display | "Calculated: £4,200 / month" with breakdown link |
...
| Type | Single-select dropdown |
| Required | Yes — only when Collection Mode = Gateway |
| Visible | Only when Collection Mode = Gateway |
| Options | Loaded from `GET /admin/v1/payment-gateways` — shows provider name (Paddle / PayHere) + environment (sandbox / production) |
| Error — empty | "Payment gateway is required for gateway collection." |
...
| Required | Yes — only when Collection Mode = Manual |
...
| Error — empty when manual | "Billing evidence or reference is required for manual collection." |
...
| Type | Number input |
| Required | Yes — only when selected plan/modules include AI capability (`chat_ai`) |
...
| Error — end before start | "Grace period end date must be after start date." |
| Error — no reason | "Reason is required for a payment exception." |
...
2. Finds the bracket matching `company_size_range` from Step 1
...
**Side effects:**
- `tenant_module_entitlements` rows created for each selected module with `status = 'provisioning'` — not yet `active`; they become `active` when tenant is activated
...
| Screenshot Capture | "Enable Screenshot Capture" | Toggle | Yes | On / Off | Default Off — requires explicit opt-in |
| Camera Photo Capture | "Enable Camera Photo Verification" | Toggle | Yes | On / Off | Default Off — requires explicit opt-in. WorkPulse tray captures a photo for identity verification events (clock-in, absence spot-check). Subject to biometric consent and retention policy. |
...
| Leave Year Start | "Leave Year Start Month" | Dropdown | Yes | January–December | Defines when annual leave entitlement resets |
...
| Carry-Forward Max Days | "Maximum Carry-Forward Days" | Number input | Required if limited | 1–365 | |
...
3. Job family templates → writes `job_families` + `job_levels`; resolves role links where possible
4. Leave policy template → writes `leave_types` + `leave_policies`; matches job levels by rank
5. App allowlist template → writes `app_allowlists`
6. Data import mapping template → writes `data_import_mapping_templates`
7. Onboarding template → writes `onboarding_templates` + `onboarding_template_tasks`
...
```json
{
  ...
  "template_applications": [
    ...
    { "template_id": "uuid", "type": "job_family", "status": "applied", "warnings": ["Level 'Senior' role link is pending — apply role template 'team-lead' to resolve."] }
  ]
}
```
...
|---|---|---|
| Organization Info | Company name, legal name, domain, tenant ID, industry, company size | "Edit" → navigates back to Step 1 in edit mode |
...
| Configuration | Work mode, monitoring settings, leave defaults, setup services list | "Edit" → Step 4 |
...
**Activation guard — backend checks before flipping status:**
...
- If `send_invite_on_activation = true` → invite email sent to admin email via Resend; `users.invite_sent_at` recorded
...
- [[developer-platform/modules/tenant-console/end-to-end-logic|Tenant Console End-to-End Logic]] — ongoing management after activation
- [[developer-platform/modules/subscription-manager/end-to-end-logic|Subscription Manager]] — plan catalog and gateway configuration
