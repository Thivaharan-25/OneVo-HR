...
- Subscription plans define reusable feature/module bundles plus employee-count/company-size pricing tiers. Company size is part of the subscription plan pricing table, not module configuration and not a separate plan identity.
- Developer Platform tenant creation stores the allowed plans, optional recommended plan, operator-selected gateway/manual collection method, setup charges, and negotiated overrides.
...
- Tenant owner does not choose Stripe, Paddle, PayHere, or Manual.
- Tenant creation does not create a trial.
- First invoice quantity is confirmed total employee count; later recurring invoices can use system snapshots after onboarding/import.
...
**Rule:** A plan can define multiple employee-count pricing tiers. The tenant owner confirmed employee count selects the tier for the first invoice.

### Section 3: Included Modules and Features
...
- Included commercial features from `module_features`
...
For each employee-count pricing tier selected in Section 2, a pricing bracket row is generated. Each bracket defines the unit prices for all selected modules at that size range.

**Price bracket row (one per employee-count pricing tier):**
...
For first invoice: quantity = tenant-owner-confirmed total employee count. Do not ask the tenant owner to split employees by monitoring or WorkSync usage during first billing.
```
...
```json
{
  ...
  "is_active": true,
  "employee_count_tiers": ["201-500", "501-1000", "1001-5000"],
  ...
  "included_feature_keys": {
    "work_management": [
      "work_management.projects",
      "work_management.tasks",
      "work_management.time_tracking",
      "work_management.analytics"
    ],
    "chat_ai": [
      "chat_ai.agentic_chat"
    ]
  },
...
| 422 | `missing_price_brackets` | Price bracket required for each selected employee-count tier |
...
The same rule applies to feature inclusion. Existing tenant subscriptions keep their snapshot of included feature keys until an operator explicitly changes that tenant's subscription/custom contract.
...
**Purpose:** Store encrypted payment gateway credentials for Stripe, Paddle, and PayHere. These credentials are used when the system automatically charges tenants or when the operator records gateway references for manually-managed payments.

**Gateway routing rule:** The ONEVO operator selects Stripe, Paddle, PayHere, or Manual during tenant commercial setup. The gateway dropdown is filtered by tenant country, configured gateway availability, supported currency, and the sales/payment policy. Tenant owners do not choose the gateway.


| Column | Description |
|---|---|
| Provider | "Stripe", "Paddle", or "PayHere" with logo |
...
Paddle is Merchant of Record. ONEVO creates the Paddle customer/subscription only after the tenant owner confirms the allowed plan, billing cycle, total employee count, and billing contact details.

1. After subscription confirmation, `InvoiceGenerationJob` creates a Paddle customer + subscription via Paddle Billing API (`POST /customers`, `POST /subscriptions`)
2. Paddle stores `paddle_customer_id` and `paddle_subscription_id` on `tenant_subscriptions`
3. The first invoice uses confirmed employee count plus setup charges; future renewals use the configured billing cycle and snapshot/contract rules
...
1. After subscription confirmation or on each `next_billing_date`, `InvoiceGenerationJob` reads `tenant_subscriptions` where `gateway = payhere`
2. Generates invoice with:
   - Line items per selected module/plan (module name, unit price, quantity, line total)
   - First invoice quantity = tenant-owner-confirmed total employee count
   - Recurring invoice quantity = billing snapshot or contract rule
...
| Payment method | Dropdown | All / Stripe / Paddle / PayHere / Manual / Waived |
...
| Employee Count Tier | Which bracket changed |
...
Stripe/PayHere tenants: ONEVO can generate or stream the invoice PDF server-side. Paddle tenants: Paddle hosts the PDF when Paddle is merchant of record; ONEVO stores `paddle_invoice_url` and redirects to it.
...
| Billed To | Tenant legal name, primary contact email, country, confirmed employee count |
...
| Gateway subscription | Stripe/Paddle/PayHere subscription updated via gateway API; next invoice reflects new rate |
...
| HTTP | Code | Condition |
|---|---|---|
| 400 | `webhook_signature_invalid` | Stripe/Paddle/PayHere webhook signature mismatch |
