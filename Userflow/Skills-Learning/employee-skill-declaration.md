...
**Related Permissions:** `skills:read` (to search existing tenant skills), `skills:validate` (skill validation by an eligible validator)
...
- **Backend:** `SkillSearchService.SearchExistingSkillsAsync()` -> [[skills]]
...
- **UI:** Click "Save". Skill request appears in profile with status badge "Pending". Toast: "Skill request submitted for validation"
...
- **Backend:** `EmployeeSkillService.DeclareSkillAsync()` -> [[skills]]
  ...
  4. Notify the eligible validator resolved through management coverage or configured reviewer permission via [[backend/notification-system|Notification System]]
  5. Publish `SkillDeclaredEvent`
  6. Create audit log entry
- **Validation:** Skill must already exist in the tenant skill library. No duplicate declarations. `proficiencyLevel` must be integer 1-5
...
- Eligible validator re-notified for validation
...
### When an eligible validator directly adds a skill to an employee's profile
...
- **API:** `POST /api/v1/employees/{employeeId}/skills`
  ...
  ```
- Creates `employee_skills` record with `status = 'validated'` and `validated_by_id = validatorEmployeeId` immediately - no pending step
- Employee receives in-app notification: "[Name] has added [Skill] to your profile at [Level]"
- **DB:** `employee_skills`, `audit_logs`
...
- [[Userflow/Skills-Learning/skill-assessment|Skill Assessment]] - eligible validator validates this declaration
...
- [[backend/notification-system|Notification System]] - validator notification on declaration
