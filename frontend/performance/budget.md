...
| Monitoring Live | <=2.0s | Charts + SignalR (heavier) |
| Time Off Calendar | <=1.5s | Calendar component (lazy) |
...
```typescript
// [ok] Correct - lazy loaded
{ path: 'monitoring', loadComponent: () => import('./features/monitoring/live-dashboard.component').then(m => m.LiveDashboardComponent) }

// [wrong] Wrong - eager import kills initial bundle
import { LiveDashboardComponent } from './features/monitoring/live-dashboard.component';
{ path: 'monitoring', component: LiveDashboardComponent }
```
...
Apply `@defer` to: org charts, work item boards, Phase 2 roadmap timelines, activity heatmaps, rich text editors, and any chart component.
