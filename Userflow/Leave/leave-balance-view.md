...
- Leave entitlements have been assigned: [[leave-entitlement-assignment|Leave Entitlement Assignment Flow]]
...
- **Backend:** `LeaveEntitlementService.GetBalanceSummaryAsync()` → [[leave]]
...
- **Backend:** `LeaveEntitlementService.GetTeamBalancesAsync()` → [[leave]]
...
- **Backend:** `LeaveReportService.ExportBalancesAsync()` → [[leave]]
...
- Can click "Adjust" to modify individual entitlements inline → [[leave-entitlement-assignment]]
...
- `AuditLogEntry` (action: `leave_balance.exported`) → [[audit-logging]] (only on CSV export)

## Related Flows

- [[leave-request-submission]] — initiate new request from balance view
- [[leave-entitlement-assignment]] — how entitlements are created
- [[leave-cancellation]] — cancelled requests restore balance

## Module References

- [[leave]] — leave module overview and architecture
- [[leave-entitlements]] — entitlement and balance data model
- [[leave-requests]] — request history data
