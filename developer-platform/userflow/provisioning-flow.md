# Tenant Provisioning - Complete 4-Step Wizard
...
The wizard collects four categories of information in sequence. After the wizard completes and the tenant is activated, all ongoing management happens from the **Tenant Detail page** - accessible by clicking any tenant row in the Tenants list.

---

## Wizard Entry

**Location:** Platform Management -> Tenants -> `+ Create Tenant` button (top-right)
...
- Step 1 data is not saved until the operator clicks **Next** at the end of Step 1 - closing the browser before that point leaves no record.
...
| Progress indicator | Step 1 of 4 highlighted - "Organization Info" |
...
| Label | "Company Name" |
| Type | Text input |
| Required | Yes - red asterisk shown |
| Placeholder | "e.g., TechNova Solutions" |
| Min length | 2 characters |
| Max length | 100 characters |
| Validation - on blur | Minimum length check |
| Validation - on submit | Non-empty, within length range |
| Error - too short | "Company name must be at least 2 characters." |
| Error - too long | "Company name cannot exceed 100 characters." |
| Error - empty | "Company name is required." |
...
| Validation - on blur | Minimum length check |
| Error - too short | "Legal company name must be at least 2 characters." |
| Error - too long | "Legal company name cannot exceed 150 characters." |
| Error - empty | "Legal company name is required." |
...
| Validation - format | Must match `^[a-z0-9]+([\-\.]{1}[a-z0-9]+)*\.[a-z]{2,}$` - lowercase only, no `https://`, no trailing slash |
| Validation - uniqueness | `GET /admin/v1/tenants/validate?domain={value}` called on blur. If domain already exists returns `409` with `"code": "domain_taken"` |
| Validation - reserved | Domains matching ONEVO internal domains (`onevo.io`, `console.onevo.io`, `app.onevo.io`) are rejected |
| Error - format | "Enter a valid domain, e.g. company.com - no https:// prefix." |
| Error - taken | "This domain is already registered to another tenant." |
| Error - reserved | "This domain is reserved and cannot be used." |
| Error - empty | "Domain is required." |
...
| Value | Auto-generated on page load - format: `TEN-YYYYMMDD-XXXX` where `YYYYMMDD` is today's date and `XXXX` is a zero-padded sequential number, e.g. `TEN-20250520-0009` |
| Help text | "A unique identifier will be used across the platform." |
| Editable | No - operator cannot change this |
...
| Note | Phase 1 serves IT companies only. Shown as a static label - not a dropdown. Multi-industry support is deferred to a future phase. Value stored as `'Technology'` in `tenants.industry`. |
| Editable | No |
...
| Character counter | Shown below - "0 / 500" updating live |
| Error - too long | "Description cannot exceed 500 characters." |
...
| Format | E.164 international format - must start with `+` and country code |
| Validation - on blur | If non-empty: must match E.164 pattern `^\+[1-9]\d{6,14}$` |
| Error - format | "Enter a valid international phone number starting with +." |
...
| Validation - on blur | If non-empty: must be a valid URL starting with `https://` |
| Error - format | "Enter a valid URL starting with https://." |
...
| Previous | Bottom-right, secondary | Disabled on Step 1 - grayed out, non-clickable |
| Next | Bottom-right, primary blue | Validates all fields -> if valid, calls `POST /admin/v1/tenants` -> on success navigates to Step 2 |
...
```json
{
  ...
  "description": "Cloud-based HR and monitoring management tools for growing tech companies.",
...
| Error - empty | "First name is required." |
| Error - too long | "First name cannot exceed 50 characters." |
...
| Min length | 1 character |
| Max length | 50 characters |
| Error - empty | "Last name is required." |
| Error - too long | "Last name cannot exceed 50 characters." |
...
| Validation - format | Standard email format `^[^@\s]+@[^@\s]+\.[^@\s]+$` |
| Validation - domain | Email domain does NOT have to match the tenant domain - some owners use personal domains. No blocking validation on domain match. |
| Validation - uniqueness within tenant | `GET /admin/v1/tenants/validate?adminEmail={value}&tenantId={id}` - same email can exist in other tenants but cannot be a duplicate pending invite for this exact tenant |
| Error - format | "Enter a valid email address." |
| Error - duplicate invite | "An invite has already been sent to this address for this tenant." |
| Error - empty | "Email address is required." |
...
| Error - empty | "Role assignment is required." |
...
| Type | Single-select dropdown (rich options - each option shows plan name, included module count, base price range) |
| Required | Yes |
| Placeholder | "Select a plan" |
| API source | `GET /admin/v1/subscription-plans` - loads all active plans |
| On change | Triggers module list refresh below and price recalculation |
| Error - empty | "Subscription plan is required." |
...
| Required | Yes |
| Options | Subscription (recurring billing) |
| Default | "Subscription" |
| On change | Billing Cycle remains visible for paid subscription setup |
...
| Error - empty | "Billing start date is required." |
| Error - past date | "Billing start date cannot be in the past." |

#### Included Modules
| Property | Value |
|---|---|
| Label | "Included Modules" |
| Type | Multi-select checkbox list - grouped by ONEVO product pillar |
| Required | At least one module must be selected |
| Source | Determined by selected plan's `subscription_plan_modules` base package rows - pre-checked base modules from plan cannot be unchecked |
...
|:---|:---|:---|
| **HR Management** | Core HR, Time Off Management, Calendar | Performance, Payroll, Skills, Learning, Recruitment, Grievance, Expense are Phase 2 - not shown |
| **Monitoring** | Activity Monitoring, Time & Attendance, Identity Verification, Productivity Analytics | App Usage Tracking is part of Activity Monitoring. Exception Engine is Phase 2. Agent Gateway is infrastructure - not a sellable module. |
| **WorkSync** | Work Management, Documents, Reports, GitHub Integration | Projects & Sprints, OKR, and Roadmaps are features within Work Management, not separate sellable modules. Chat and Chat AI are Phase 2. |
| **Foundation (always included, not selectable)** | Authentication, Tenant Configuration, Roles & Permissions, Notifications, Org Structure | Pre-checked and locked. Cannot be unchecked. Provisioned automatically for every tenant. |

**Approval routing in Phase 1:** Provisioning seeds lightweight module-owned approval settings only. Time Off, attendance correction, overtime, transfer, promotion, and position-access approvals use Org Structure management coverage and Notifications. Full Workflow / Automation Engine configuration is Phase 2.
...
| Display | "Calculated: GBP 4,200 / month" with breakdown link |
...
| Type | Currency input |
| Required | No |
| Placeholder | "Keep blank to use calculated price" |
...
| Type | Textarea |
| Required | Yes - only when Override Price is filled |
| Min length | 10 characters |
| Max length | 500 characters |
| Error - empty when override set | "Reason is required when overriding the calculated price." |

#### Payment Gateway
| Property | Value |
|---|---|
| Label | "Payment Gateway" |
| Type | Read-only resolved field with optional operator refresh |
| Required | Yes |
| Source | Active `payment_gateway_country_routes` row for the tenant country and current environment |
| Display | Provider name (Stripe / Paddle / PayHere), gateway label, environment |
| Error - no route | "No active payment gateway route is configured for this tenant country." |
| Note | Tenant owner does not choose the gateway. Gateway routes are configured in System Config -> Payment Gateways. |
...
| Type | Number input |
| Required | Yes - only when selected plan/modules include a Phase 2 AI capability |
| Visible | Hidden in Phase 1 unless a future AI-capable module is selected |
| Min value | 1,000 |
| Max value | 100,000,000 |
| Unit | Tokens per month |
| Placeholder | "e.g., 500000" |
| Error - empty when AI selected | "AI token limit is required when AI-capable modules are included." |
| Error - below min | "AI token limit must be at least 1,000 tokens per month." |
...
| Required | Yes - for all tenants |
| Visible | Always shown |
| Min value | 1 GB |
| Max value | 1,000 TB |
| Note | Storage is a **single shared pool for the entire tenant** - not split per module. All modules draw from it: uploaded files, HR documents, screenshots, payslips, verification photos, attachments. Stored as `tenant_storage_limit_gb` on `tenant_subscriptions`. |
| Error - empty | "Tenant storage limit is required." |
...
| Maintenance Rate | "Annual Maintenance Rate (%)" | Number input (0-100) | Yes | Percentage of license fee |
| Maintenance Renewal Date | "Maintenance Renewal Date" | Date picker | Yes | Annual renewal date |
| Maintenance Collection | "Maintenance Collection Mode" | Radio: Gateway / Waived | Yes | How recurring maintenance is collected |
...
5. If Override Price is entered, stored as `override_price`; calculated price stored as `calculated_price` - both preserved in DB
...
```json
{
  ...
  "selected_module_keys": ["core_hr", "time_off", "time_attendance", "activity_monitoring", "worksync_projects"],
  "payment_gateway_config_id": "gw-paddle-global-prod",
...
**Side effects:**
- `tenant_module_entitlements` rows created for each selected module with `status = 'provisioning'` - not yet `active`; they become `active` only after first payment succeeds
...
| Note | Only configuration relevant to the modules selected in Step 3 is shown. If Monitoring is not selected, monitoring settings are hidden. |

### Section: Work Mode

| Field | Label | Type | Required | Options | Notes |
|---|---|---|---|---|---|
| Work Mode | "Primary Work Mode" | Radio | Yes | Hybrid, Remote, On-site | Drives default policy suggestions for monitoring, time_off, and device management |

### Section: Monitoring & Privacy (shown only if Activity Monitoring or Time & Attendance selected)

| Field | Label | Type | Required | Options | Notes |
|---|---|---|---|---|---|
| Monitoring Transparency Mode | "Employee Transparency Mode" | Radio | Yes | Full Transparency (employees see all monitoring data), Summary Only (employees see summaries), Hidden (employees see no monitoring data) | Compliance-critical - operators must choose consciously |
| Desktop Agent Transparency | "Desktop Agent Visibility" | Radio | Yes | Visible to employees (tray icon always shown), Silent (no tray icon) | Local law may require visible agent - operator is responsible |
| Data Collection Scope | "Collect Application Usage" | Toggle | Yes | On / Off | Default On when monitoring modules selected |
| Screenshot Capture | "Enable Screenshot Capture" | Toggle | Yes | On / Off | Default Off - requires explicit opt-in for authorized on-demand screenshot capture |
| Auto Screenshot Capture | "Enable Auto Screenshot on Deviation" | Toggle | Yes | On / Off | Default Off - when enabled, monitoring can request screenshots automatically after a detected deviation; tenant does not configure each deviation trigger separately |
| Camera Photo Capture | "Enable Camera Photo Verification" | Toggle | Yes | On / Off | Default Off - requires explicit opt-in. WorkPulse tray captures identity photos for clock-in, clock-out, on-demand reviewer request, or absence-detected verification. Clock-in/out photo prompts are scoped by `verification.photo_capture_context_scope` (`remote_only`, `onsite_only`, `remote_and_onsite`, `disabled`). Subject to biometric consent and retention policy. |
| Input Counting | "Enable Keyboard/Mouse Activity Counting" | Toggle | Yes | On / Off | Counts only - never captures content |
...
| Work Week Days | "Work Week" | Multi-select (Mon-Sun) | Yes | Default Mon-Fri |
...
### Section: Monitoring Policy (shown only if Activity Monitoring or Time & Attendance selected)

| Field | Label | Type | Required | Options | Notes |
|---|---|---|---|---|---|
| Monitoring Policy Template | "Monitoring Policy Template" | Dropdown | No | Loaded from `GET /admin/v1/configuration-templates?type=monitoring_policy` - auto-selects template matching tenant `industry_profile_tag` if one exists | Seeds `monitoring_feature_toggles` for this tenant; individual toggles can be adjusted after apply |
| Monitoring Transparency Mode | "Employee Transparency Mode" | Radio | Yes | Full Transparency, Summary Only, Hidden | Compliance-critical - operators must choose consciously |
...
### Section: Time Off Policy (shown only if Time Off Management selected)

| Field | Label | Type | Required | Options | Notes |
|---|---|---|---|---|---|
| Time Off Policy Template | "Time Off Policy Template" | Dropdown | No | Loaded from `GET /admin/v1/configuration-templates?type=time_off_policy` | Seeds `time_off_types` and `time_off_policies`; assignment is configured as tenant, department, or position scope |
| Time Off Year Start | "Time Off Year Start Month" | Dropdown | Yes | January-December | Defines when annual Time Off entitlement resets |
| Time Off Approval | "Time Off Approval Routing" | Radio | Yes | Management coverage routing, Auto-approved | Default for new employees |
| Carry-Forward Policy | "Time Off Carry-Forward" | Radio | Yes | No carry-forward, Limited (max hours input), Unlimited | |
| Carry-Forward Max Hours | "Maximum Carry-Forward Hours" | Number input | Required if limited | Positive decimal | |
...
**Assignment rule:** Monitoring Policy, App Allowlist, Onboarding, and Time Off Policy templates can apply to the entire tenant, selected departments, or selected positions. If multiple templates match an employee, resolve in this order: position-specific, department-specific, tenant-global, system default.
...
| Column | Description |
|---|---|
| Service Name | Human-readable name, e.g. "Initial Org Structure Setup", "Time Off Policy Configuration" |
...
**Rule:** Free setup services connected to selected modules are auto-added to the checklist. Operator cannot remove free services - they can only mark them complete. Paid services are opt-in additions.
...
```json
{
  ...
  "time_off": {
    "time_off_year_start_month": 1,
    "default_annual_time_off_minutes": 12000,
    "time_off_approval": "manager_required",
...
1. Configuration template -> writes `tenant_settings`
2. Monitoring policy template -> writes `monitoring_feature_toggles`
3. Position template pack -> writes departments + positions; materializes linked role templates with tenant-entitlement-filtered permissions
4. Time Off policy template -> writes `time_off_types` + `time_off_policies` and assignment rows
5. App allowlist template -> writes `app_allowlists` and assignment rows
6. Data import mapping template -> writes `data_import_mapping_templates`
7. Onboarding template -> writes `checklist_templates`
...
- `time_off_types` + `time_off_policies` rows (if Time Off policy template applied)
- `app_allowlists` rows (if app allowlist template applied)
- `data_import_mapping_templates` rows (if data import template applied)
- `checklist_templates` rows (if onboarding template applied)
...
This is not a data-entry step. It is a read-only summary of all choices from Steps 1-4.

### Screen Layout

| Section | What Is Shown | Edit Link |
|---|---|---|
| Organization Info | Company name, legal name, domain, tenant ID, industry, estimated employee count | "Edit" -> navigates back to Step 1 in edit mode |
| Admin Account | Admin name, email, role, invite-on-activation setting | "Edit" -> Step 2 |
| Subscription Plan | Plan name, commercial model, billing cycle, billing start date, selected modules list, calculated price, override price if any, payment mode, gateway if applicable | "Edit" -> Step 3 |
| Configuration | Work mode, monitoring settings, Time Off defaults, setup services list | "Edit" -> Step 4 |
| Warnings | Any incomplete optional items (e.g. "2 setup services marked Not Started") shown as yellow callout - not blocking |

### Upgrade Button

| Element | Value |
|---|---|
| Button label | "Upgrade to {Plan Name}" - dynamically shows the subscription plan selected in Step 3 (e.g., "Upgrade to Enterprise Plan") |
...
**Request body:** `{}` - no body required; all data already saved in previous steps

**Upgrade guard - backend checks before first invoice generation:**
...
| `step_1_complete = true` through `step_4_complete = true` | `provisioning_incomplete` | "All wizard steps must be completed before the first invoice can be generated." |
...
| No active payment gateway route for tenant country | `missing_payment_gateway_route` | "No active payment gateway route is configured for this tenant country." |

**On all checks passed:**

Clicking Upgrade generates the first invoice and sets the tenant to `pending_payment`. The tenant does not become `active` until the first payment succeeds.
...
```json
{
  ...
  "first_invoice_id": "inv-...",
  "invoice_url": "https://..."
}
```

**State written on Upgrade:**
- `tenants.status` -> `'pending_payment'`
- `tenant_module_entitlements` rows remain `status = 'provisioning'` - they do **not** become `active` until first payment succeeds
- First invoice generated from selected subscription terms and linked to tenant
- Tenant role materialized from role template - `roles` and `role_permissions` rows created
- Audit log entry: `action = 'tenant.provisioning_confirmed'`, actor = platform account ID, target = tenant ID, timestamp

### State Written on First Payment Success

When the payment gateway confirms the first invoice is paid:

- `tenants.status` -> `'active'`
- `tenants.activated_at` -> current timestamp
- `tenant_module_entitlements` rows updated: `status` -> `'active'` for all provisioning entitlements
- `tenant_subscriptions.status` -> `'active'`
- Payment and invoice records linked to the activation event
- If `send_invite_on_activation = true` and invite not already sent -> invite email sent to admin email via Resend; `users.invite_sent_at` recorded
- Audit log entry: `action = 'tenant.activated'`, actor = system (payment webhook), target = tenant ID, timestamp

**Error response (422 Unprocessable Entity) when provisioning checks fail:**
```json
{
  ...
  "blockers": [
    ...
    { "code": "missing_payment_gateway_route", "message": "No active payment gateway route is configured for this tenant country." }
  ]
}
```

Frontend renders the blockers as a red callout list above the Upgrade button.

### Post-Upgrade State (Pending Payment)

After Upgrade generates the first invoice:
- Tenant appears in Tenants list with blue **Pending Payment** badge
- Yellow "In Progress" badge is removed
- Tenant is **not** visible to `/api/v1/*` tenant-facing endpoints until first payment succeeds
- First invoice is awaiting payment via the configured payment gateway

### Post-Activation State (After First Payment)
...
- Module entitlements are active
...
2. Clicking the row calls `GET /admin/v1/tenants/{id}` - response includes `provisioning_state`
...
- [[developer-platform/modules/tenant-console/end-to-end-logic|Tenant Management End-to-End Logic]] - ongoing management after activation
- [[developer-platform/modules/subscription-manager/end-to-end-logic|Subscription Plans]] - plan catalog and gateway configuration
- [[developer-platform/modules/module-catalog-manager/end-to-end-logic|Module Catalog Manager]] - ONEVO product module pricing and permission ownership
- [[developer-platform/modules/role-template-manager/end-to-end-logic|Role Template Manager]] - role templates applied during Manage/Configure
- [[developer-platform/auth|Authentication & Authorization]] - platform-admin JWT and impersonation model
