...
Work Collaboration extends the shared `documents` table with workspace/project scope, adds an approval workflow with document locking, wiki pages for structured knowledge, and `task_documents` for linking documents to tasks. The `documents` table is shared infrastructure - HR Documents and Work Collaboration coexist in the same table.
...
- [[Userflow/Work-Management/collaboration-flow|Work Collaboration]] - workspace documents and wiki user flow
