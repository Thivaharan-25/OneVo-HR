...
**Architecture:** Clean Architecture â€” Domain (no deps) â†’ Application (MediatR + FluentValidation) â†’ Infrastructure (EF Core + Npgsql) â†’ two ASP.NET Core host projects. All tests run without a real PostgreSQL instance.
...
- Test: `tests/ONEVO.Tests.Unit/Domain/ResultTests.cs` (stub â€” implemented in Task 2)
...
- [ ] **Step 3: Run the test â€” verify it FAILS (types don't exist yet)**

```powershell
dotnet test tests/ONEVO.Tests.Unit/ONEVO.Tests.Unit.csproj
```

Expected: build error â€” `ONEVO.Domain.Common` namespace not found.
...
Edit `tests/ONEVO.Tests.Unit/ONEVO.Tests.Unit.csproj` â€” add Domain reference:
...
- [ ] **Step 12: Run the test â€” verify it PASSES**
...
git commit -m "feat(domain): add shared kernel â€” BaseEntity, ValueObject, Errors, Enums, ValueObjects"
```
...
- [ ] **Step 2: Run â€” verify FAILS (types missing)**
...
- [ ] **Step 6: Run Result tests â€” verify PASS**
...
git commit -m "feat(domain): add foundation entities â€” Country, Tenant, User, FileRecord"
```
...
- [ ] **Step 11: Run DbContext boot test â€” verify PASSES**
...
## Task 5: EF Migration â€” InitialFoundation
...
Expected: build error â€” Program class not accessible.
...
public class TenantResolutionMiddleware
{
    ...
    // Tenant is resolved from backend-held auth/session state by CurrentUserService.
    // Custom domain â†’ tenant lookup added in Phase 2.
...
- [ ] **Step 7: Run API boot test â€” verify PASSES**
...
git commit -m "feat(api): rewrite Program.cs â€” health endpoint, correlation ID, exception mapping, Serilog, tenant resolution middleware"
```
...
- [ ] **Step 6: Run Admin boot test â€” verify PASSES**
...
git commit -m "feat(admin-api): rewrite Program.cs â€” /admin/v1/health, correlation ID, exception mapping, Serilog"
```
...
- [ ] **Step 4: Re-run tests â€” confirm still passing**
...
```powershell
git add .
git commit -m "chore: complete DEV1 Task 1 â€” backend foundation implemented, all tests passing"
```
...
**Spec coverage check:**
- âœ… Solution structure â€” 9 projects, wired
- âœ… Shared kernel â€” BaseEntity, PlatformEntity, Result<T>, IDomainEvent, ITenantContext, IDateTimeProvider
- âœ… API health checks and OpenAPI â€” `/api/v1/health`, `/admin/v1/health`
- âœ… DbContext PostgreSQL + snake_case â€” all configurations use `HasColumnName("snake_case")`
- âœ… Migrations â€” `InitialFoundation` covering 4 foundation tables
- âœ… Pipeline â€” CorrelationId, ExceptionMapping, TenantResolution middleware; Serilog
- âœ… Tests â€” boot tests for both APIs + DbContext boot test

**Placeholder scan:** None found â€” all steps contain concrete code.
