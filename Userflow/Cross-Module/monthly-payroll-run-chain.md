...
**Required Permission(s):** `payroll:run`, `payroll:approve`, `attendance:read`, `time_off:read`  
**Modules Involved:** Payroll, Time & Attendance, Time Off, Employee-Management, Expense, Documents, Notifications

---

## Context

Payroll doesn't just "run salaries." It must collect data from attendance (overtime, absences), Time Off (unpaid Time Off deductions), expense (reimbursements), and employee management (new hires, terminations, salary changes). This doc maps every data source that feeds into a payroll run and the outputs it produces.
...
- Attendance data finalized for the period (no pending corrections) -> [[Userflow/Time-Attendance/attendance-correction|Attendance Correction]]
- Time Off data finalized (no pending approvals affecting payroll) -> [[Userflow/Time-Off/time-off-approval|Time Off Approval]]
...
|:------|:-------|:-------------|:---------------|
| 1 | **Time-Attendance** | Attendance summary pulled for pay period: total hours, overtime hours, absences, late arrivals | Hours worked, overtime, unpaid absences |
| 2 | **Time Off** | Time Off records pulled: unpaid Time Off hours/deducted hours, encashment requests where supported | Unpaid deductions, encashment additions |
...
|:------|:-------|:-------------|:----------------|
| 6 | **Payroll** | Gross pay calculated: base salary + overtime + allowances + reimbursements - unpaid Time Off | `PayrollCalculationCompleted` |
...
| 15 | **Time Off** | Time Off encashment balances updated (if any encashments were processed) | `TimeOffBalanceUpdated` |
...
+-- Time Off balance update (Step 15) - independent
...
| Time Off data has pending approvals | Unpaid Time Off deductions may be incorrect | Configured Time Off approver completes approvals; authorized payroll user re-runs |
...
- [[Userflow/Time-Attendance/overtime-management|Overtime Management]] - overtime data source
- [[Userflow/Time-Off/time-off-cancellation|Time Off Cancellation]] - late cancellations affecting payroll
