...
| `time_off_types` (Time Off module) | Write on apply - Time Off policy template payload |
| `time_off_policies` (Time Off module) | Write on apply - Time Off policy template payload |
| `app_allowlists` (Configuration module) | Write on apply - app allowlist template payload |
| `checklist_templates` (Core HR module) | Write on apply - onboarding/offboarding checklist template payload |
...
| `time_off_policy` | `time_off_types`, `time_off_policies` | `time_off` |
| `monitoring_policy` | `monitoring_feature_toggles` | `monitoring` |
| `app_allowlist` | `app_allowlists` | `configuration` |
| `onboarding` | `checklist_templates`; optional assignment to tenant, departments, or positions | `core_hr` |
...
- `template_key` is the stable machine-readable identifier (e.g. `uk-standard-time_off`). The `name` is the display label. Both must be unique globally.
...
- Monitoring Policy, App Allowlist, Onboarding, and Time Off Policy templates use assignment scopes: tenant, department, or position. Position-specific assignment wins over department-specific, which wins over tenant-global.
