...
- `developer-platform/modules/configuration-template-manager/testing.md` — 36 test cases covering creation, payload validation, all 7 apply handlers, deferred role linking, entitlement guard, reapply behaviour, deactivation guard, audit
...
- `database/schemas/org-structure.md`
  - `job_levels` table: added `job_family_id`, `salary_minimum`, `salary_maximum`, `default_role_id`, `pending_role_template_id` columns; added unique constraint; documented deferred role-linking rule in full
