...
**Area:** Auth & Access  
**Trigger:** User navigates to `https://{tenantSlug}.onevo.com` (any unauthenticated visit - every session start)  
...
- **UI:** Browser navigates to `https://{tenantSlug}.onevo.com`. While branding loads: base dark background (`#09090b`), no flash of unstyled content (branding applied before first paint via server-side fetch). Navbar appears: tenant logo + name left, `System Live` status pill + `Sign In ->` button right
...
- **UI:** `--primary` CSS variable overridden with `tenant.primaryColor` on `document.documentElement` (if set). Tenant logo displayed in navbar (32x32, rounded). Tenant name shown in navbar and hero footer. If no `primaryColor`: falls back to default `#7c3aed` (Violet Electric)
...
- `POST /api/v1/auth/refresh` -> refreshed session metadata -> redirect to `/dashboard` immediately
