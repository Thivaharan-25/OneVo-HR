...
1. **HR Management** - Employee lifecycle, Time Off, org structure, payroll, skills, compliance
2. **Monitoring** - Desktop activity monitoring, presence tracking, biometric verification, exception detection, productivity analytics
3. **Work** - Phase 1 simple projects, work items, basic project membership, simple project settings, worklogs, and simple docs/pages where retained. Planner, sprints/boards, roadmaps, OKR, Chat/Chat AI, GitHub/repository automation, and Teams sync are Phase 2 design references. All Work data stays in the same backend and database as HR; no bridge APIs.
...
- Monitoring: "Is this employee present and productive? Any anomalies?"
...
| **Frontend** | Angular 21, TypeScript, Angular Router, Angular Material, Angular Signals - merged customer app shell + internal `dev-console` + `shared` library |
...
| **Caching** | Phase 1 `IMemoryCache`; Redis only if a future distributed/multi-instance deployment requires it |
...
| 5 | **Shared Platform** | SSO, notifications, lightweight Phase 1 approvals, subscription plans, entitlements; Workflow Engine is Phase 2 | [[modules/shared-platform/overview\|Shared Platform]] |
...
| 8 | **Time & Attendance** | Clock in/out, biometric events, breaks, unified presence sessions | [[modules/time-attendance/overview\|Time & Attendance]] |
| 9 | **Identity Verification** | Photo/biometric verification, on-demand capture from manager alerts | [[modules/identity-verification/overview\|Identity Verification]] |
...
| 11 | **Monitoring Alerts / Exception Engine** | Phase 1 lightweight alerts through Notifications; full anomaly rule engine is Phase 2 | [[modules/exception-engine/overview\|Exception Engine]] |
| 12 | **Notifications** | In-app, email (Resend), SignalR real-time push | [[modules/notifications/overview\|Notifications]] |
| 13 | **Time Off** | Time Off types, policies, entitlements, approval workflow | [[modules/time-off/overview\|Time Off]] |
| 14 | **Calendar** | Company events, public holidays, Time Off-conflict checks | [[modules/calendar/overview\|Calendar]] |
...
|:--|:-------|:--------|:------|:-----|
| W1 | **Work Foundation** | Workspaces, workspace_members, workspace_roles | DEV5 | [[modules/work-management/foundation\|WS Foundation]] |
| W2 | **Project Management** | Phase 1 projects, project members, simple settings, labels, and retained milestones; project_workspaces, epics, versions, and release_calendar are Phase 2 references | DEV5 | [[modules/work-management/projects\|WS Projects]] |
...
| W4 | **Planning** | Phase 2: sprints, boards, roadmaps, and advanced planning | Phase 2 | [[modules/work-management/planning\|Planning]] |
| W5 | **OKR** | Phase 2: objectives, key results, check-ins | Phase 2 | [[modules/work-management/okr\|OKR]] |
| W6 | **My Space** | Phase 2 personal boards, todos, saved views | Phase 2 | [[modules/work-management/my-space\|My Space]] |
| W7 | **Time Management** | Time_logs, timesheets, timesheet_entries | DEV5 | [[modules/work-management/time\|WS Time]] |
| W8 | **Resource Management** | Phase 2 capacity/resource planning | Phase 2 | [[modules/work-management/resources\|Resources]] |
| W9 | **Chat** | Phase 2 Work Chat and Teams sync | Phase 2 | [[modules/work-management/chat\|Chat]] |
| W10 | **Chat AI** | Phase 2 AI action detection and reminders | Phase 2 | [[modules/work-management/chat-ai\|Chat AI]] |
...
| W12 | **Insight & Analytics** | Phase 2 dashboards, chart widgets, sprint reports, report snapshots, dashboard shares, and saved views | Phase 2 | [[modules/work-management/analytics\|Analytics]] |
| W13 | **Integration & API** | Phase 2 repository integration and automation rules | Phase 2 | [[modules/work-management/integrations\|Integrations]] |
...
**Supported Phase 2 tag entities:** `task`, `sprint`, `project`, `chat`, `time`, `time_off`, `doc`, `review`, `notify`, `board`, `okr`
...
@time-off:request 2026-05-05 2026-05-07 "Conference"
...
Week 1 (Foundation - all 8 devs in parallel):
  ...
  DEV5: Work Foundation (workspaces, workspace_members, workspace_roles)
...
Week 2 (Core modules):
  ...
  DEV3: Time & Attendance (shifts, schedules, clock-in/out)
  DEV4: Time & Attendance - Biometric integration
  DEV5: Projects + project_members + simple settings/labels/milestones; project_workspaces, epics, versions, and release_calendar are Phase 2 references
  DEV6: Work item foundations; boards/sprints are Phase 2 planning references
  DEV7: Chat AI is Phase 2 reference only unless reactivated
  DEV8: wiki_pages + document_approvals + task_documents (documents locked_at/locked_by fields)

Week 3 (Intelligence + planning):
  DEV1: Time Off (time_off_types, time_off_requests, time_off_balances, approval workflow)
  DEV2: Lightweight monitoring alerts + Notifications; full Exception Engine Phase 2
  ...
  DEV5: Worklogs / Time Management for Phase 1; OKR is Phase 2
  DEV6: Work item reporting refinements; sprint planning, burndown, roadmaps, and roadmap items are Phase 2
  DEV7: Phase 2 reminders + My Space references only unless reactivated
  DEV8: Phase 2 GitHub/GitLab integration references only unless reactivated

Week 4 (Analytics + AI + CI):
  DEV1: Productivity Analytics (aggregation jobs, dashboards, report exports)
  DEV2: Lightweight alert notification follow-up; full Exception Engine Phase 2
  ...
  DEV5: Resource Management is Phase 2 reference only unless reactivated
  DEV6: Simple Work reporting only if retained; advanced analytics and sprint reports are Phase 2
  DEV7: Chat AI is Phase 2 reference only unless reactivated
  DEV8: repository/CI records and task automation rules are Phase 2 references only unless reactivated
...
- DEV3 Org Structure before DEV5 Work Foundation (legal_entities in workspaces)
- Work foundation before Work items; boards and chat are Phase 2 references
- DEV8 Documents before DEV8 task_documents (documents table must exist)
- Boards, sprints, and burndown snapshots are Phase 2 references
- Chat and Chat AI are Phase 2 references
...
All 38 modules share one `ApplicationDbContext`. Work tables (`projects`, `tasks`, and Phase 2 `sprints`, `boards`, `channels`, etc.) are EF Core entities exactly like HR tables. `backend/bridge-api-contracts.md` is **DEPRECATED**. Do not implement it.
...
`ai_action_jobs` is Phase 2 design reference for reversible AI-triggered actions.

### Monitoring Lifecycle
Desktop agent data collection is controlled by clock-in/break/clock-out from Time & Attendance. No data captured before clock-in, during breaks, or after clock-out. GDPR requirement. See [[modules/agent-gateway/monitoring-lifecycle/overview|Monitoring Lifecycle]].
...
- **Dev 1:** [[current-focus/DEV1-infrastructure-setup|Infrastructure]], [[current-focus/DEV1-core-hr-profile|Core HR Profile]], [[current-focus/DEV1-time_off|Time Off]], [[current-focus/DEV1-productivity-analytics|Productivity Analytics]]
...
- **Dev 3:** [[current-focus/DEV3-org-structure|Org Structure]], [[current-focus/DEV3-time-attendance-setup|Time & Attendance]], [[current-focus/DEV3-activity-monitoring|Activity Monitoring]], [[current-focus/DEV3-calendar|Calendar]], [[current-focus/DEV3-skills-core|Skills Core]]
- **Dev 4:** [[current-focus/DEV4-shared-platform-agent-gateway|Shared Platform + Agent Gateway]], [[current-focus/DEV4-time-attendance-biometric|Time & Attendance Biometric]], [[current-focus/DEV4-identity-verification|Identity Verification]], [[current-focus/DEV4-configuration|Configuration]]
...
- **Dev 5:** [[current-focus/DEV5-wms-foundation|Work foundation + projects + worklogs; OKR/resources are Phase 2 references]]
- **Dev 6:** [[current-focus/DEV6-tasks-boards-planning|Work items; boards/planning/roadmaps are Phase 2 references]]
- **Dev 7:** [[current-focus/DEV7-chat-ai-reminders|Chat/Chat AI/My Space/Reminders are Phase 2 references unless reactivated]]
