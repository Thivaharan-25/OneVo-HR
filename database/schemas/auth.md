...
**Foreign Keys:** `tenant_id` -> [[database/schemas/infrastructure#`tenants`|tenants]], `user_id` -> [[database/schemas/infrastructure#`users`|users]], `created_by_user_id` -> [[database/schemas/infrastructure#`users`|users]], `created_by_platform_user_id` -> [[developer-platform/database/schema#platform_users|platform_users]], `revoked_by_user_id` -> [[database/schemas/infrastructure#`users`|users]], `revoked_by_platform_user_id` -> [[developer-platform/database/schema#platform_users|platform_users]]

**Rules:** completing with password uses `invited_email`. Completing with Google may use a different verified Google email only when `allow_google_email_mismatch = true` and the domain is allowed. Mismatches are audit-logged and the original `invited_email` remains on this record. Exactly one `created_by_*` actor is set for human-created invites; exactly one `revoked_by_*` actor is set when revoked by a human actor.
...
|:-------|:-----|:------|
| `id` | `uuid` | PK |
| `template_key` | `varchar(100)` | Globally unique stable key for reusable references, e.g. `tenant-owner`, `engineering-manager` |
...
**Uniqueness rule:** One override row exists per tenant/user/permission.

**Coverage rule:** Permission overrides grant or revoke a permission only. They do not define employee visibility or approval routing. Management coverage is the single source for employee visibility and Phase 1 approval routing.
...
**Source rule:** Position access grant rules generate real `user_roles` rows. The generated row is the active grant after confirmation or approval; the position rule itself is never treated as an active permission grant. Tenant-admin UX uses "Role granted" from the position access rule. **Can manage employees in** is stored separately in Org Structure `management_coverage_records`.

---

## `access_grant_requests`

Phase 1 lightweight approval records for sensitive position-based access generated during onboarding, transfer, promotion, or position assignment. This table is not a Workflow Engine instance.
...
| `position_access_template_id` | `uuid` | FK -> position_access_templates; internal position access grant rule |
...
**Routing rule:** Phase 1 approver resolution uses Org Structure `management_coverage_records` in this order: Position coverage, Department coverage, Company-wide coverage, then Routing issue. Within each covered target, try Primary owner, Backup owner 1, Backup owner 2, and so on. Assign the approval to one eligible owner only. If no valid owner has the required permission, create a routing issue with: "No eligible owner could approve this request. Check position coverage and permissions." Workflow Engine routing is Phase 2.

**Coverage rule:** Management coverage is the single source for employee visibility and Phase 1 approval routing. Role assignments and permission overrides do not store employee coverage.
