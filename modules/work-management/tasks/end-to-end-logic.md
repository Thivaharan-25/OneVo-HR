...
PATCH /api/v1/tasks/{id}/status
  ...
  -> UpdateTaskStatusHandler
    ...
    -> 5. Publish TaskStatusChangedEvent (notifications in Phase 1; sprint burndown and reminders only when Phase 2 planning/My Space are enabled)
    -> 6. If status = "done": also check for linked reminder (`reminders.linked_task_id`)
         -> if found: mark reminder complete
    -> Return Result<TaskDto>
```

## Move Card on Board (Phase 2 Planning)
