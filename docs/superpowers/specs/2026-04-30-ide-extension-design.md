...
```json
{
  ...
  "permitted_tag_actions": ["task:new", "task:view", "task:status", "task:assign", "task:comment", "task:link", "task:move", "sprint:add", "sprint:start", "time:log", "time:start", "time:stop", "time_off:request", "time_off:view", "time_off:cancel", "clockin", "break:start", "break:end", "clockout"],
...
- HR section in picker is shown only when `active_modules` includes at least one HR module such as `core_hr`, `time_off`, or `calendar`
...
```
+---------------------------------+
| [search] Search actions or describe... |
+---------------------------------+
| HR AUTOMATION        [tenant]   |  <- only if active_modules includes an HR module
|   [active] @clockin        start shift (instant)
|   [break] @break:start    take a break (instant)
|   [critical] @break:end      end break (instant)
|   [critical] @clockout       end shift (instant)
|   [form] @time-off:request  apply Time Off -> form
|   [form] @time-off:view     view balance
|   [form] @time-off:cancel   cancel request -> picker
|   [pay] @payslip:view   view payslip
|   [time] @overtime:request -> form
|   [report] @timesheet:view
|   [submit] @timesheet:submit
|   [calendar] @shift:view     today/week schedule
|   [calendar] @calendar:view  holidays + events
+---------------------------------+
| TASKS                           |
|   [ok] @task:new       create task -> form
|   [view] @task:view      view task detail
|   [update] @task:status    update status -> picker
|   [assign] @task:assign    assign to member -> picker
|   [comment] @task:comment   add comment -> input
|   [link] @task:link      link branch/doc
|   -> @task:move      move to sprint/board
+---------------------------------+
| SPRINT                          |
|   + @sprint:add     add task to sprint
|   [start] @sprint:start   start sprint
|   [ok] @sprint:complete complete sprint
|   [view] @sprint:view    view sprint
+---------------------------------+
| TIME                            |
|   [time] @time:log       log hours -> form
|   [start] @time:start     start timer (instant)
|   [stop] @time:stop      stop + log (instant)
|   [view] @time:view      view time logs
+---------------------------------+
| MORE                            |
|   [board] @board:move     move task to column
|   [target] @okr:checkin    OKR progress update
|   [doc] @doc:link       link doc to task
|   [doc] @doc:create     create document
|   [review] @review:request request code review
|   [bell] @notify:send    send notification
|   [project] @project:view   view project
+---------------------------------+
```
...
- Clicking an action either:
  ...
  - **Opens a mini form** inline above the composer (time_off:request, task:new, time:log, overtime:request, timesheet:submit)
...
Fields: Time Off Type (select from tenant time off types), From date, To date, Reason (optional)  
Shows: Remaining balance for selected Time Off type  
...
Confirmation card appears in chat:
  +---------------------------------+
  | [ai] OneVo AI detected action     |
  | @time:log 2h , Fix login bug    |
  | Sprint 12 , TASK-123            |
  | [[ok] Confirm] [[edit] Edit] [[dismiss] Dismiss] |
  +---------------------------------+
                    v
...
|---|---|---|
| "I want to take Time Off next week" | `@time-off:request` | Opens mini form pre-filled with next week's dates |
...
- Branch created -> `[branch] Arun created branch: feature/task-123-fix`
- Commit -> `[commit] Committed: "Fix JWT validation"`
- PR opened -> `[pr] PR #42 opened by Arun -> [Review PR]`
- CI passed -> `[ok] CI Passed - PR #42`
- CI failed -> `[wrong] CI Failed - PR #42 , [View Logs]`
- PR merged -> `[merged] PR #42 merged -> Task -> Done`
- Branch naming mismatch -> `[warning] Branch mismatch: expected feature/task-123-*`
...
- [ ] HR tag actions route through existing module endpoints:
  - `@clockin` -> `POST /api/v1/time-attendance/clock-in`
  - `@break:start` -> `POST /api/v1/time-attendance/breaks/start`
  - `@break:end` -> `PATCH /api/v1/time-attendance/breaks/{id}/end`
  - `@clockout` -> `POST /api/v1/time-attendance/clock-out`
  - `@overtime:request` -> `POST /api/v1/time-attendance/overtime`
  - `@time-off:request` -> `POST /api/v1/time-off/requests`
  - `@time-off:view` -> `GET /api/v1/time-off/entitlements/me`
  - `@time-off:cancel` -> `DELETE /api/v1/time-off/requests/{id}`
  ...
  - `@shift:view` -> `GET /api/v1/time-attendance/schedules/current`
  - `@calendar:view` -> `GET /api/v1/calendar?scope=company`
- [ ] All HR tag actions are included in `permitted_tag_actions[]` only if user has the relevant HR permission

**VS Code Extension:**
- [ ] On entitlements load: if `active_modules` includes `core_hr`, `time_off`, or `calendar` -> show HR AUTOMATION section in picker
...
- [ ] Mini form for `@time-off:request`: Time Off Type (from `GET /api/v1/time-off/types`), From, To, Reason. Shows balance.
- [ ] Mini form for `@overtime:request`: Date, Hours, Reason
- [ ] Mini form for `@timesheet:submit`: Shows period summary, confirm to submit
- [ ] View tags (`@time-off:view`, `@payslip:view`, `@timesheet:view`, `@shift:view`, `@calendar:view`) -> open detail in WebviewPanel

**Acceptance Criteria:**
- HR section visible in picker when `active_modules` includes `core_hr`, `time_off`, or `calendar`; absent otherwise
...
- `@time-off:request` form shows remaining balance for selected Time Off type
- Submitting a Time Off request creates an approval workflow, bot card shows in chat: "Time Off request submitted - Pending approval"
...
- "take Time Off" / "apply for Time Off" / "I need {N} days off" -> `@time-off:request`
...
- "I want to take Time Off next week" -> suggestion card -> Edit opens Time Off form pre-filled with next week dates
...
| HR tag routing | IDEExtension tag executor | Routes to TimeAttendance + Time Off endpoints |
| `GET /api/v1/time-off/types` | Time Off | Return tenant Time Off types for form dropdown |
| `GET /api/v1/time-off/entitlements/me` | Time Off | Return hour-based entitlement balance per type for user |
...
|   +-- TimeOffFormWebview.ts         # @time-off:request mini form
...
2. **HR section only when `active_modules` includes an HR module such as `core_hr`, `time_off`, or `calendar`.** No HR UI for non-HR tenants - not even a hint.
