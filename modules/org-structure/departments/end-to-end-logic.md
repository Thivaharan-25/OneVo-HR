...
POST /api/v1/org/departments
  -> DepartmentController.Create(CreateDepartmentCommand)
    ...
    -> DepartmentService.CreateAsync(command, ct)
      ...
      -> 2. Validate name uniqueness within legal entity
      -> 3. Validate code uniqueness within legal entity (if provided)
      ...
      -> 5. If head_position_id provided:
           ...
           c. Validate position belongs to same legal entity as department
...
```
GET /api/v1/org/departments?legalEntityId={id}&view=tree|flat
...
PUT /api/v1/org/departments/{id}
  ...
  -> DepartmentService.UpdateAsync(id, command, ct)
    ...
    -> 2. If parent_department_id changed:
         ...
         b. Verify new parent belongs to same legal entity as this department
         c. Detect circular references using CTE:
            Walk from new parent up to root; if current dept id found -> reject
    -> 3. If code changed, re-validate uniqueness within legal entity
...
| Duplicate name in legal entity | 409 | "Department name already exists in this legal entity" |
| Duplicate code in legal entity | 409 | "Department code already exists in this legal entity" |
| Parent not found | 422 | "Parent department not found" |
| Parent in different legal entity | 422 | "Parent department must belong to the same legal entity" |
...
| Head position in different legal entity | 422 | "Head position must belong to the same legal entity" |
