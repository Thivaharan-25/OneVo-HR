...
- **UI:** Workforce -> Devices -> "Register Device" -> enter: device ID, name, physical office location (e.g., "Main Entrance"), type (fingerprint/face/NFC)
- **API:** `POST /api/v1/workforce/devices`
- **DB:** `biometric_devices` record created with `office_location_id` pointing to `office_locations`
...
If a registered biometric device or office location has a verified outage, an admin can enable a time-limited fallback:
