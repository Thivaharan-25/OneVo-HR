...
**Foreign Keys:** `tenant_id` â†’ [[database/schemas/infrastructure#`tenants`|tenants]], `created_by_id` â†’ [[database/schemas/infrastructure#`users`|users]]
...
**Foreign Keys:** `workflow_instance_id` â†’ [[#`workflow_instances`|workflow_instances]], `workflow_step_id` â†’ [[#`workflow_steps`|workflow_steps]], `actor_id` â†’ [[database/schemas/core-hr#`employees`|employees]], `delegated_to_id` â†’ [[database/schemas/core-hr#`employees`|employees]], `workflow_step_assignment_id` -> [[#`workflow_step_assignments`|workflow_step_assignments]]
...
**Foreign Keys:** `tenant_id` â†’ [[database/schemas/infrastructure#`tenants`|tenants]], `requested_by_id` â†’ [[database/schemas/infrastructure#`users`|users]], `target_user_id` â†’ [[database/schemas/infrastructure#`users`|users]]

---

## `escalation_rules`

> **Scope:** Workflow SLA timeouts â€” fires when a workflow item (leave request, expense claim, etc.) sits in a pending state longer than `sla_hours`. See [[database/schemas/exception-engine#`escalation_chains`|escalation_chains]] for alert routing on system-detected anomalies â€” that is a separate system.
...
| `escalate_to_role_id` | `uuid` | FK â†’ roles (nullable) |
| `notification_template_id` | `uuid` | FK â†’ notification_templates |
| `is_active` | `boolean` |  |
| `created_by_id` | `uuid` | FK â†’ users |
| `created_at` | `timestamptz` |  |

**Foreign Keys:** `tenant_id` â†’ [[database/schemas/infrastructure#`tenants`|tenants]], `escalate_to_role_id` â†’ [[database/schemas/auth#`roles`|roles]], `notification_template_id` â†’ [[#`notification_templates`|notification_templates]], `created_by_id` â†’ [[database/schemas/infrastructure#`users`|users]]

---

## `global_app_catalog`

Global catalog of known applications managed by the OneVo dev team via the developer platform. No `tenant_id` â€” shared across all tenants. HR admins browse this when configuring app allowlists.

| Column | Type | Notes |
|:-------|:-----|:------|
| `id` | `uuid` | PK |
| `app_name` | `varchar(200)` | e.g., "Google Chrome" |
| `process_name` | `varchar(100)` | e.g., "chrome.exe" â€” authoritative matching key used by ingest processor |
...
| `created_by_id` | `uuid` | FK â†’ dev_platform_accounts |
| `created_at` | `timestamptz` | |
| `updated_at` | `timestamptz` | |

**Foreign Keys:** `created_by_id` â†’ `dev_platform_accounts`

**Index:** `process_name` UNIQUE â€” no duplicate process names in catalog.
...
**Foreign Keys:** `tenant_id` â†’ [[database/schemas/infrastructure#`tenants`|tenants]], `office_location_id` â†’ [[database/schemas/org-structure#`office_locations`|office_locations]]
...
**Foreign Keys:** `tenant_id` â†’ [[database/schemas/infrastructure#`tenants`|tenants]], `placed_by_id` â†’ [[database/schemas/infrastructure#`users`|users]], `released_by_id` â†’ [[database/schemas/infrastructure#`users`|users]]
...
**Foreign Keys:** `tenant_id` â†’ [[database/schemas/infrastructure#`tenants`|tenants]], `configured_by_id` â†’ [[database/schemas/infrastructure#`users`|users]]
...
**Foreign Keys:** `plan_id` â†’ [[#`subscription_plans`|subscription_plans]]
...
**Unique constraint:** `(permission_code)` â€” one permission can be owned by only one module.
...
Reusable setup templates managed in the Developer Platform â†’ Configuration Template Manager. Platform operators create and version these globally; applying a template to a tenant seeds the corresponding module tables without mutating the global template.
...
| `name` | `varchar(150)` | Display name |
| `description` | `varchar(500)` | Nullable â€” human-readable summary shown in the template picker |
...
| `industry_profile_tag` | `varchar(50)` | Nullable â€” links monitoring policy templates to an industry for auto-selection during provisioning |
| `payload_json` | `jsonb` | Type-specific template content â€” schema defined per `template_type` in the Configuration Template Manager end-to-end-logic doc |
...
**Note on `role` type:** Role templates are managed separately via the Role Template Manager module and stored in `role_templates`, not here. The `job_family` payload references `role_templates.id` per level for the deferred role-linking mechanism â€” see `job_levels.pending_role_template_id`.

**Foreign Keys:** `created_by_id` â†’ [[developer-platform/database/schema#dev_platform_accounts|dev_platform_accounts]]

---

## `tenant_configuration_template_applications`

Audit record of every template application to a tenant. One row per apply action â€” reapplying creates a new row, not an update.

| Column | Type | Notes |
|:-------|:-----|:------|
| `id` | `uuid` | PK |
| `tenant_id` | `uuid` | FK â†’ tenants |
| `configuration_template_id` | `uuid` | FK â†’ configuration_templates |
...
| `applied_payload_json` | `jsonb` | Snapshot of the payload that was applied â€” immutable after creation |
| `custom_payload_json` | `jsonb` | Nullable â€” tenant-specific overrides made after application; does not mutate the global template |
| `warnings_json` | `jsonb` | Array of warning strings returned at apply time, e.g. unresolved job level rank references |
| `status` | `varchar(20)` | `applied` â†’ `customized` (if tenant edited) â†’ `superseded` (if reapplied) â†’ `removed` |
| `applied_by_id` | `uuid` | FK â†’ dev_platform_accounts |
| `applied_at` | `timestamptz` | |
| `updated_at` | `timestamptz` | Nullable â€” set when status changes or custom payload is edited |
...
**Foreign Keys:** `tenant_id` â†’ [[database/schemas/infrastructure#tenants|tenants]], `configuration_template_id` â†’ [[#configuration_templates|configuration_templates]], `applied_by_id` â†’ [[developer-platform/database/schema#dev_platform_accounts|dev_platform_accounts]]
...
**Foreign Keys:** `user_id` â†’ [[database/schemas/infrastructure#`users`|users]], `tenant_id` â†’ [[database/schemas/infrastructure#`tenants`|tenants]], `replaced_by_id` â†’ [[#`refresh_tokens`|refresh_tokens]]
...
**Foreign Keys:** `user_id` â†’ [[database/schemas/infrastructure#`users`|users]], `tenant_id` â†’ [[database/schemas/infrastructure#`tenants`|tenants]]
...
**Foreign Keys:** `tenant_id` â†’ [[database/schemas/infrastructure#`tenants`|tenants]], `subscription_id` â†’ [[#`tenant_subscriptions`|tenant_subscriptions]]
...
**Unique:** `(tenant_id, snapshot_date)` â€” one snapshot per tenant per month.
...
**Foreign Keys:** `updated_by_id` â†’ [[database/schemas/infrastructure#`users`|users]]
...
**Foreign Keys:** `tenant_id` â†’ [[database/schemas/infrastructure#`tenants`|tenants]], `logo_file_id` â†’ [[database/schemas/infrastructure#`file_records`|file_records]], `updated_by_id` â†’ [[database/schemas/infrastructure#`users`|users]]
...
**Foreign Keys:** `tenant_id` â†’ [[database/schemas/infrastructure#`tenants`|tenants]], `plan_id` â†’ [[#`subscription_plans`|subscription_plans]], `created_by_id` â†’ [[database/schemas/infrastructure#`users`|users]]
...
**Foreign Keys:** `tenant_id` â†’ [[database/schemas/infrastructure#`tenants`|tenants]], `webhook_endpoint_id` â†’ [[#`webhook_endpoints`|webhook_endpoints]]
...
**Foreign Keys:** `tenant_id` â†’ [[database/schemas/infrastructure#`tenants`|tenants]], `workflow_definition_id` â†’ [[#`workflow_definitions`|workflow_definitions]], `initiated_by_id` â†’ [[database/schemas/core-hr#`employees`|employees]]
...
**Foreign Keys:** `workflow_instance_id` â†’ [[#`workflow_instances`|workflow_instances]], `workflow_step_id` â†’ [[#`workflow_steps`|workflow_steps]], `assigned_to_id` â†’ [[database/schemas/core-hr#`employees`|employees]] (legacy only)
...
**Foreign Keys:** `workflow_definition_id` â†’ [[#`workflow_definitions`|workflow_definitions]], `approver_role_id` â†’ [[database/schemas/auth#`roles`|roles]] (legacy only)
